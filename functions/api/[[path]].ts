...
async function generate(env: Env, system: string, prompt: string, maxOutputTokens: number) {
  if (env.GROQ_API_KEY) {
    ...
    if (!response.ok) {
      console.error('Groq response failed', response.status, data?.error?.code || data?.error?.type || 'unknown');
...
function assertPublicUrl(raw: string) {
  ...
  if (isBlockedHost(url.hostname)) throw new Error('PRIVATE_URL');
  return url;
}
...
async function merchantSupport(input: any) {
  ...
  let product: URL;
  try { product = assertPublicUrl(String(input.productUrl)); }
  ...
  return json({
    ...
    emails: Array.from(emailSources.entries()).slice(0, 10).map(([email, source]) => ({ email, source })),
  });
}

async function claimAssist(env: Env, input: any) {
  const arabic = input?.lang === 'ar';
  ...
  try {
    const text = await generate(env, `${systemBase} ${arabic ? 'Respond in clear Arabic.' : 'Respond in clear English.'}`, `${taskInstruction}\n\nClaim facts:\n${serialized}`, 900);
...
async function claimPackage(env: Env, input: any) {
  const arabic = input?.lang === 'ar';
  const serialized = JSON.stringify(input?.claim ?? {});
  if (serialized.length > 12000) return json({ error: 'Claim context is too large' }, 400);
  try {
...
async function claimChat(env: Env, input: any) {
  const arabic = input?.lang === 'ar';
  ...
  const history = Array.isArray(input?.messages)
    ? input.messages.slice(-12).filter((m: any) => m && (m.role === 'user' || m.role === 'assistant') && typeof m.content === 'string' && m.content.length <= 3000)
    : [];
  ...
  try {
    const text = await generate(
      env,
      `${systemBase} ${arabic ? 'Respond in clear Arabic.' : 'Respond in clear English.'} If official website content is supplied, identify an email address ONLY if it appears verbatim in that checked official-site content. If no email is present, say you could not verify one; never guess. When asked, draft a professional claim email based only on the claim facts, with a subject line and concise factual body.`,
      `Claim facts:\n${claim}${websiteContext}\n\nConversation so far:\n${transcript}\n\nAnswer the user's latest message. Preserve context from earlier turns. Give a complete response and never cut off mid-sentence.`,
      1400,
    );
...
export const onRequest = async (context: PagesContext) => {
  ...
  if (request.method === 'GET' && endpoint === 'health') return json({ ok: true, service: 'resolverelay-cloudflare' });
  ...
  if (endpoint === 'claim-assist') return claimAssist(env, input);
  if (endpoint === 'merchant-support') return merchantSupport(input);
  if (endpoint === 'claim-package') return claimPackage(env, input);
  if (endpoint === 'claim-chat') return claimChat(env, input);
