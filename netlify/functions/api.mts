...
async function assertPublicUrl(raw: string) {
  ...
  if (!host || host === 'localhost' || host.endsWith('.localhost') || host.endsWith('.local')) throw new Error('PRIVATE_URL');
  if (isIP(host) && privateAddress(host)) throw new Error('PRIVATE_URL');
  if (!isIP(host)) {
    const addresses = await lookup(host, { all: true, verbatim: true });
    if (!addresses.length || addresses.some(x => privateAddress(x.address))) throw new Error('PRIVATE_URL');
  }
  return url;
}
...
export default async (req: Request) => {
  ...
  if (endpoint === 'claim-package') return claimPackage(input);
  if (endpoint === 'claim-chat') return claimChat(input);
